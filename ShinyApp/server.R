...
bigram_df <- read.csv("data/bigram_df_dis_1.csv")
trigram_df <- read.csv("data/trigram_df_dis_1.csv")

##########################################################
# preprocessing user input                               #
##########################################################

clean <- function(wordAB = "THE last"){
  # converting to lower case
  wordAB <- tolower(wordAB)
  # checking word B is available in bigram list
  wordB <- strsplit(wordAB, " ")[[1]][2]
  match = bigram_df$wordA == wordB
  warning = "none"
  if(sum(match) == 0)
  {
...
shinyServer(function(input, output) {
  ...
  wordAB_warning <- reactive({clean(input$wordAB)})
  # Getting word prediction and most likely word
  get_prediction <- reactive({comb_prediction(wordAB_warning()[1])})
